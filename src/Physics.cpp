...
bool Physics::PreUpdate()
{
	...
	// Fixed-timestep accumulator. Box2D's solver is not timestep-independent
	// and its docs require a fixed step; dt (already seconds since L02) is
	// whatever the frame took, so we step the world in fixed FIXED_TIMESTEP
	// chunks and carry the remainder. MAX_STEPS is not optional: without it,
	// a long stall (e.g. a breakpoint -- see the L03 slide) makes the
	// accumulator huge, the catch-up loop runs hundreds of steps, that takes
	// longer still, and the accumulator grows further -- the spiral of death.
	...
	if (steps == MAX_STEPS) accumulator = 0.0f; // drop the backlog
...
PhysBody* Physics::CreateCapsule(int x, int y, int width, int height, bodyType type)
{
	...
	def.type = ToB2Type(type);
	def.position = { PIXEL_TO_METERS(x), PIXEL_TO_METERS(y) };
	def.fixedRotation = true; // character bodies don't tip over

	b2BodyId b = b2CreateBody(world, &def);

	const float radius = PIXEL_TO_METERS(width) * 0.5f;
	const float segmentHalfLength = PIXEL_TO_METERS(height - width) * 0.5f;

	b2Capsule capsule;
	capsule.center1 = { 0.0f, -segmentHalfLength };
	capsule.center2 = { 0.0f,  segmentHalfLength };
...
void Physics::AddSensorShape(PhysBody* p, int offsetX, int offsetY, int w, int h)
{
	if (!p || B2_IS_NULL(p->body)) return;

	const b2Vec2 center = { PIXEL_TO_METERS(offsetX), PIXEL_TO_METERS(offsetY) };
	const b2Polygon box = b2MakeOffsetBox(PIXEL_TO_METERS(w) * 0.5f, PIXEL_TO_METERS(h) * 0.5f, center, b2Rot_identity);
	...
	b2CreatePolygonShape(p->body, &sdef, &box);
}
...
int PhysBody::RayCast(int x1, int y1, int x2, int y2, float& normal_x, float& normal_y) const
{
	...
	b2WorldId worldId = b2Body_GetWorld(body);
	b2QueryFilter qf = b2DefaultQueryFilter();

	const b2RayResult res = b2World_CastRayClosest(worldId, p1, d, qf);
	if (!res.hit) return -1;
...
void Physics::DrawCircleCb(b2Vec2 center, float radius, b2HexColor /*color*/, void* /*ctx*/)
{
	...
	r.DrawCircle(METERS_TO_PIXELS(center.x), METERS_TO_PIXELS(center.y),
		METERS_TO_PIXELS(radius) * Engine::GetInstance().window->GetScale(),
		255, 255, 255);
}
